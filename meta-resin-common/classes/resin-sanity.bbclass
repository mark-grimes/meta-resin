...
def resinos_build_configuration():
	...
	if d.getVar('RESIN_CONNECTABLE') or d.getVar('RESIN_CONNECTABLE_SERVICES') or d.getVar('RESIN_CONNECTABLE_ENABLE_SERVICES'):
		bb.warn("Your build configuration uses RESIN_CONFIGURABLE* variables. These variables are no longer used. There is only one type of resinOS image type which is unconnected by default. The os-config tool is used to configure the resinOS image for connectivity to a resin instance.")
	return success
